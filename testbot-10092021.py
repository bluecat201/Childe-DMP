...
TOKEN = 'NTkyMzQ2ODg1ODQzNDUxOTMz.XQ-ABg.z-tq4tmljm1tqsCjSK2xl0XwtXw'

client = discord.Client(intents=intents)

@client.event
async def on_ready():
    await client.change_presence(activity=discord.Streaming(name='alpha v0.0.6', url='https://www.twitch.tv/Bluecat201'))    
...
async def on_message(message):
    ...
    if user_message.lower() == 'hello':
        ...
        return
    elif user_message.lower() == 'dobré ráno':
...
async def ban(ctx, member: discord.member , *,reason):
    ctx.channel.send(f"zabanovàn {member}")
    member.ban()


@client.event
async def on_private_channel_deleted(channel):
    server = str(channel.guild.name)
    print(f'Roomka "{channel}" byla smazána na serveru "{server}"')

@client.event
async def on_private_channel_create(channel):
    server = str(channel.guild.name)
    print(f'Roomka "{channel}" byla vytvořena na serveru "{server}"')
""" 
